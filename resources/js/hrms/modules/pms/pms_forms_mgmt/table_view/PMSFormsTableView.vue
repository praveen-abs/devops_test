<template>
  <div>
    ...
    <Dialog
      ...
      v-model:visible="canShowPMSFormUsage_Dialog"
    ...
    >
      ...
      <div
        class="confirmation-content d-flex justify-content-start align-items-center mt-3 ml-3"
      >
        <DataTable
          :value="pmsFormsMgmtStore.array_pms_forms_usage_details.data"
        ...
        >
          <Column header="Calendar Type/Year" style="width: 20%">
            <template #body="slotProps">
              <span
                ><b
                  >{{ slotProps.data.abbrevation }} :
                  {{ dayjs(slotProps.data.start_date).format("YYYY") }} -
                  {{ dayjs(slotProps.data.end_date).format("YYYY") }}</b
                ></span
              >
            </template>
          </Column>
          <Column field="assignment_period" header="Period" style="width: 5%">
            <template #body="slotProps">
              <div>
                <span
                  ><b>{{ slotProps.data.assignment_period.toUpperCase() }}</b></span
                >
              </div>
            </template>
          </Column>
    ...
    <Dialog
      ...
      v-model:visible="canShowPMSFormTemplate_Dialog"
    ...
    >
      ...
      <div
        class="confirmation-content d-flex justify-content-center align-items-center mt-3 ml-3"
      >
        <DataTable
          :value="pmsFormsMgmtStore.pms_form_template_details.data"
          scrollable
          scrollHeight="400px"
        ...
        >
          ...
          <Column
            v-for="col of availableTableColumns_PMSTemplateDetails"
            :key="col.field"
            :field="col.field"
            style="width: 25%"
            :header="col.header"
    ...
    <div>
      <DataTable
        :value="pmsFormsMgmtStore.array_pms_forms_authors_list"
              ...
              <Column header="Form Assignment Details" sortable>
                <template #body="slotProps">
                  <div v-if="slotProps.data.is_pmsform_used == 1">
                    <Button
                      label="View"
                      @click="getPMSFormUsageDetails(slotProps.data)"
                      class="btn btn-orange"
                      style="height: 2em"
                      raised
                    />
              ...
              <Column header="Actions" sortable>
                <template #body="slotProps">
                  <Button
                    severity="success"
                    label="View Form"
                    @click="showPMSFormTemplateDetails(slotProps.data)"
                    class="btn btn-orange"
                    style="height: 2em"
                    raised
                  />
