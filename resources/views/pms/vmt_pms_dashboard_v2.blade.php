...
@section('script')
    ...
    <script>
        ...
        $('body').on('click', '.plus-sign', function() {
            var showdimension = "{{ $pmsConfig['dimension'] == 'true' ? 'block' : 'none' }}";
            var showkpi = "{{ $pmsConfig['kpi'] == 'true' ? 'block' : 'none' }}";
            var showoperational = "{{ $pmsConfig['operational'] == 'true' ? 'block' : 'none' }}";
            var showmeasure = "{{ $pmsConfig['measure'] == 'true' ? 'block' : 'none' }}";
            var showfrequency = "{{ $pmsConfig['frequency'] == 'true' ? 'block' : 'none' }}";
            var showtarget = "{{ $pmsConfig['target'] == 'true' ? 'block' : 'none' }}";
            var showstretchTarget = "{{ $pmsConfig['stretchTarget'] == 'true' ? 'block' : 'none' }}";
            var showsource = "{{ $pmsConfig['source'] == 'true' ? 'block' : 'none' }}";
            var showkpiWeightage = "{{ $pmsConfig['kpiWeightage'] == 'true' ? 'block' : 'none' }}";
            ...
            if (showdimension == 'block') {
                dimension =
                    '<td class="text-box-td p-1"><textarea name="dimension[]" id="" class="text-box" cols="20" placeholder="type here"></textarea></td>';
            } else {
                dimension = '<input type="hidden" name="dimension[]">';
            }
            if (showkpi == 'block') {
                kpi =
                    '<td class="text-box-td p-1"><textarea name="kpi[]" id="" class="text-box" cols="20" placeholder="type here"></textarea></td>';
            } else {
                kpi = '<input type="hidden" name="kpi[]">';
            }
            if (showoperational == 'block') {
                operational =
                    '<td class="text-box-td p-1"><textarea name="operational[]" id="" class="text-box" cols="20" placeholder="type here"></textarea></td>';
            } else {
                operational = '<input type="hidden" name="operational[]">';
            }
            if (showmeasure == 'block') {
                measure =
                    '<td class="text-box-td p-1"><textarea name="measure[]" id="" class="text-box" cols="20" placeholder="type here"></textarea></td>';
            } else {
                measure = '<input type="hidden" name="measure[]">';
            }
            if (showfrequency == 'block') {
                frequency =
                    '<td class="text-box-td p-1"><textarea name="frequency[]" id="" class="text-box" cols="20" placeholder="type here"></textarea></td>';
            } else {
                frequency = '<input type="hidden" name="frequency[]">';
            }
            if (showtarget == 'block') {
                target =
                    '<td } class="text-box-td p-1"> <textarea name="target[]" id="" class="text-box" cols="20" placeholder="type here"></textarea></td>';
            } else {
                target = '<input type="hidden" name="target[]">';
            }
            if (showstretchTarget == 'block') {
                stretchTarget =
                    '<td class="text-box-td p-1"><textarea name="stretchTarget[]" id="" class="text-box" cols="10" placeholder="type here"></textarea></td>';
            } else {
                stretchTarget = '<input type="hidden" name="stretchTarget[]">';
            }
            if (showsource == 'block') {
                source =
                    '<td class="text-box-td p-1"><textarea name="source[]" id="" class="text-box" cols="10" placeholder="type here"></textarea></td>';
            } else {
                source = '<input type="hidden" name="source[]">';
            }
            if (showkpiWeightage == 'block') {
                kpiWeightage =
                    '<td class="text-box-td p-1"><textarea name="kpiWeightage[]" id="" class="text-box" cols="10" placeholder="type here"></textarea></td>';
            } else {
