        ...
        <div class="row">
            <div class="col-sm-12 col-md-12 col-xl-12 col-xl-12 mt-2 mb-4">
                ...
                <div class="d-flex align-items-center ">
                    @if ($checked && $checked->checkin_time)
                        @if($checked->checkout_time)
                            <i class="ri-time-line text-warning mr-2"></i><span id="check_timing" class="f-13 w-20">Check Out : {{date('H:i', strtotime($checked->checkout_time))}}</span>
                        @else                        
                            {{-- If not check_out time, then user havent checked-out yet --}}
                            <i class="ri-time-line text-warning mr-2"></i><span id="check_timing" class="f-13 w-20">Check In : {{date('H:i', strtotime($checked->checkin_time))}}</span>
                        @endif
                    @endif
                </div>
            </div>
...
<div class="position-fixed top-0 end-0 p-3" style="z-index: 1080">
    <div id="toast_notification" class="toast toast-border-success overflow-hidden mt-3" role="alert" aria-live="assertive" aria-atomic="true">
    ...
    $('#checkin_function').change(function() {
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(checked_status)
                    $('#check_timing').html("Check In: "+ data['time'].split(" ")[1]);//get the time only
                else
                    $('#check_timing').html("Check Out: "+ data['time'].split(" ")[1]);//get the time only
