        ...
        try {
            ...
            foreach ($user as $singleUser) {
                ...
                if ($attendance_setting_details['lc_status'] == 1) {
                    ...
                    array_push($header_2, 'Total LC Hours');
                }
                if ($attendance_setting_details['eg_status'] == 1) {
                ...
                if ($attendance_setting_details['lc_status'] == 1) {
                    $total_LC_hrs = CarbonInterval::minutes($total_LC_mins)->cascade()->forHumans();
                    $total_LC_mins = $total_LC_mins . ' Minutes';
                    array_push($arrayReport, $total_LC, $total_LC_mins, $total_LC_hrs);
                }
                if ($attendance_setting_details['eg_status'] == 1) {
                    array_push($arrayReport, $total_EG);
                }
    ...
    public function fetchConsolidateReportData($start_date, $end_date, $department_id, $client_id, $active_status)
    {
        ...
        try {
            ...
            foreach ($user as $singleUser) {
                ...
                $arrayReport = array($singleUser->user_code, $singleUser->name, $singleUser->designation, Carbon::parse($singleUser->doj)->format('d-M-Y'));
                $array_Report = array($singleUser->user_code, $singleUser->name, $singleUser->designation, Carbon::parse($singleUser->doj)->format('d-M-Y'));
                ...
                foreach ($attendanceResponseArray as $key => $value) {
                    ...
                    if ($doj->gt($current_date)) {
                        ...
                        if ($shift_settings->is_lc_applicable == 1 ||  $shift_settings->is_eg_applicable == 1) {
                            ...
                            array_push($array_Report,  $lc_eg_day_att);
                            $total_present++;
                        } else {
                            array_push($array_Report, 'P');
                            $total_present++;
                        }
                ...
                array_push($array_Report, $total_weekoff, $total_holidays, $total_OT, $total_present, $total_absent, $total_lop, $total_leave, $total_halfday, $total_OD,);
                if ($attendance_setting_details['lc_status'] == 1) {
                    $total_LC_hrs = CarbonInterval::minutes($total_LC_mins)->cascade()->forHumans();
                    $total_LC_mins = $total_LC_mins . ' Minutes';
                    array_push($array_Report, $total_LC, $total_LC_mins, $total_LC_hrs);
                }
                if ($attendance_setting_details['eg_status'] == 1) {
                    array_push($array_Report, $total_EG);
                }
                $total_payable_days = ($total_weekoff + $total_holidays + $total_present + $total_leave + $total_halfday) - $total_lop;
                array_push($array_Report,  $total_payable_days);
                array_push($reportresponse, $array_Report);
                unset($array_Report);
            }

            $data = array($header_2, $reportresponse);
dd($data);
            return $data;
        } catch (\Exception $e) {
            $response = [
