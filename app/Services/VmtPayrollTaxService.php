...
class VmtPayrollTaxService
{
    ...
    public function fetchInvestmentsReports($active_status, $regime_type)
    {
        ...
        $Employee_details = User::join('vmt_employee_details', 'vmt_employee_details.userid', '=', 'users.id')
            ...
            ->whereIn('vmt_employee_statutory_details.tax_regime', $regime_type)
            //->where('users.client_id', $client_id)
            ->whereIn('users.id', $inv_emp);

if($Employee_details->exists()){

    $Employee_details=$Employee_details->get(['users.id as user_id', 'users.user_code as Employee Code', 'users.name as Employee Name', 'vmt_employee_details.gender as Gender', 'vmt_employee_details.pan_number as PAN Number', 'vmt_employee_details.dob as Date Of Birth',
                                              'vmt_employee_details.doj as Date Of Joining', 'vmt_employee_statutory_details.tax_regime as Tax Regime']);
   ...
   array_push($salary_data['headers'], "Employee Code", "Employee Name", "Gender", "PAN Number", "Date Of Birth", "Date Of Joining", "Tax Regime");
        ...
        $reports_data['headers'] = $salary_data['headers'];

        $reports_data['rows'] = $salary_data['rows'];
