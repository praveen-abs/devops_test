...
<<<<<<<< HEAD:bootstrap/ssr/NormalOnboardingMainStore46681.js
import { S as Service } from "./Service46681.js";
========
import { S as Service } from "./Service92532.js";
>>>>>>>> 3d11572c6ee7fb534efc658b88a39b370fca8e71:bootstrap/ssr/NormalOnboardingMainStore92532.js
import { inject, ref, reactive, computed } from "vue";
import useValidate from "@vuelidate/core";
import { required, helpers, email } from "@vuelidate/validators";
import axios from "axios";
import { useToast } from "primevue/usetoast";
const useNormalOnboardingMainStore = defineStore("useNormalOnboardingMainStore", () => {
  ...
  const getPersonalDocuments = (e, filename) => {
    ...
    if (selectedDocument) {
      if (selectedDocument.type == "image/jpeg" || selectedDocument.type == "image/png" || selectedDocument.type == "application/pdf") {
        ...
        } else if (filename == "DrivingLicense") {
        ...
        } else if (filename == "RelievingLetter") {
  ...
  const submit = (isSubmitted) => {
    ...
    axios.post("/vmt-employee-onboard", formData).then((response) => {
  ...
  const compensatory_calculation = () => {
    ...
    employee_onboarding.special_allowance = employee_onboarding.basic - employee_onboarding.hra;
  ...
  const net_calculation = () => {
    employee_onboarding.net_income = employee_onboarding.gross - employee_onboarding.epf_employee - employee_onboarding.esic_employee;
  };
  const gross_calculation = () => {
    employee_onboarding.total_ctc = employee_onboarding.gross + employee_onboarding.epf_employer_contribution + employee_onboarding.esic_employer_contribution + employee_onboarding.insurance + employee_onboarding.graduity;
  ...
  const special_allowance_cal = () => {
    let total = employee_onboarding.statutory_bonus + employee_onboarding.child_education_allowance + employee_onboarding.food_coupon + employee_onboarding.other_allowance;
    let sa = employee_onboarding.special_allowance;
    console.log(total, sa);
    console.log(sa - total);
    setTimeout(() => {
  ...
  const graduity = () => {
    let total = employee_onboarding.total_ctc;
    console.log("total" + total);
    let sum = parseInt(employee_onboarding.total_ctc) + parseInt(employee_onboarding.graduity);
    console.log(sum);
  ...
  return {
    ...
    maritalDetails,
    bloodGroups,
    checkIsQuickOrNormal,
    family_details_disable,
    isSpouseDisable,
    spouseDisable,
    ForCopyAdrress,
    spouseGenderCheck,
    ...
    is_ac_no_exists,
    is_mobile_no_exists,
    personal_mail_exists,
    pan_card_exists,
    isQuickOrBulkOnboarding,
    populateQuickOnboardData,
    compensatory_calculation,
    net_calculation,
    gross_calculation,
    statutory_bonus,
    special_allowance_cal,
    child_allowance,
    food_coupon,
    lta,
    other_allowance,
...
export {
  useNormalOnboardingMainStore as u
};
