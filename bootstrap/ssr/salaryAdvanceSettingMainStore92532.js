...
const salaryAdvanceSettingMainStore = defineStore("salaryAdvanceSettingMainStore", () => {
  ...
  const toSelectoption = (flow, value) => {
    ...
    if (flow == 4) {
      console.log(value);
      var itemsWithoutCurrent = dropdownlist.value.filter(function(x) {
        return x.name == value;
      });
    ...
    var itemsWithoutCurrent = filteredApprovalFlow.value.filter(function(x) {
      return x.val !== value.val;
    });
  ...
  async function getInterestFreeAndInterestWithLoanHistory(Status) {
    let status = Status;
    axios.post(
      "/interest-and-interestfree-loan-settings-details",
      {
        Status: status
      }
    ).then((res) => {
  ...
  return {
    ...
    option2,
    dropdownlist,
    filteredApprovalFlow,
    toSelectoption,
    isSalaryAdvanceFeatureEnabled,
    eligibleSalaryAdvanceEmployeeData,
    sa,
    SalaryAdvanceFeatureApprovalFlow,
    saveSalaryAdvanceFeature,
    create_new_from,
    getCurrentStatus,
    client_name_status,
    sendClient_code,
    salaryAdvanceHistory,
    salaryAdvanceSettingsDetails,
    SalaryEmpDetails,
    ...
    lwif,
    saveLoanWithInterest,
    blink_UI,
    swalFunction,
    sal_adv_reset,
    resetIfl,
    RestLwif,
    getInterestFreeAndInterestWithLoanHistory,
    interestFreeLoanHistory,
    interestwithLoanHistory,
    EnableAndDisable,
    interstfreeloanPage,
...
export {
  salaryAdvanceSettingMainStore as s
};
